...
def get_viz_data():
    ...
    if assay_id is not None:
        if wc_id == "-1" or wc_id is None:
            rows, cols, well_conditions, wc_ID_list, max_fluorescence_val = wcModel.get_assay_viz_dataGrid(assay_id)
            return json.dumps({"result": well_conditions, "Xs": rows, "Ys": cols, "wc_ID_list": wc_ID_list, "max_val": max_fluorescence_val}, use_decimal=True)
        else:
